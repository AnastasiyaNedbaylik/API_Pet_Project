...
def test_register_user():
    ...
    assert status == 200, f"Получен статус {status} вместо 200"
...
def test_register_user_with_incorrect_emails():
    ...
    assert status == 422, f"Получен статус {status} вместо 200"
...
def test_register_user_with_inv_passwords():
    ...
    assert status == 400, f'Получен статус {status} вместо 200'
...
def test_login_with_invalid_email():
    ...
    assert status == 400, f'Получен статус {status} вместо 400'
    assert returned_response == 'Username is taken or pass issue'
...
def test_dd_pet_without_req_data():
    ...
    assert status == 422, f'Получен статус {status} вместо 422'
...
def test_get_pet():
    ...
    assert status == 200
    assert isinstance(pet_data, dict), "Данные не соответствуют ожидаемой структуре"
